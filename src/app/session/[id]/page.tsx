                      ...
                      <article key={product.id} className="flex items-center gap-3 rounded-md bg-muted px-3 py-2.5">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center justify-between gap-3">
                            <div className="min-w-0"><p className="truncate text-sm font-semibold">{product.nombre}</p>{product.sku && <p className="truncate text-xs text-muted-foreground">SKU {product.sku}</p>}</div>
                            <p className="shrink-0 text-sm font-bold">{formatCurrency(product.precio * product.cantidad)}</p>
                          </div>
                          <div className="mt-1.5 flex flex-wrap items-center gap-1.5 text-xs text-muted-foreground">
                            <Badge variant="outline" className="h-5 px-1.5 text-xs">{t.session.qty(product.cantidad)}</Badge>
            ...
            {!isLive && <Card>
              ...
              <CardContent className="space-y-3">
                <div className="rounded-md bg-muted p-4">
                  ...
                  {!isLive && !hasPaidInitial && <form className="mt-4 space-y-3" onSubmit={startInitialPayment}><div className="space-y-1.5"><Label htmlFor="delivery-city">{t.session.deliveryCity(country.name)}</Label><DeliveryCityCombobox cities={country.cities} defaultValue={suggestedDeliveryCity} /><p className="text-xs text-muted-foreground">{t.session.deliveryCityHint(country.adjective[locale])}</p></div><div className="space-y-1.5"><Label htmlFor="delivery-address">{t.session.deliveryAddress}</Label><Textarea className="min-h-20 resize-y" id="delivery-address" name="address" defaultValue={session.deliveryAddress || ""} minLength={8} maxLength={500} rows={3} autoComplete="street-address" placeholder={t.session.deliveryAddressPlaceholder} required /><p className="text-xs text-muted-foreground">{t.session.deliveryAddressHint}</p></div><p className="text-xs text-muted-foreground">{t.session.addressLockNote}</p>{paymentErrorMessage && <p className="text-sm text-destructive">{paymentErrorMessage}</p>}<Button className="w-full" disabled={!hasProducts || paymentStarting}>{paymentStarting ? t.session.openingPayment : t.session.continueAndPay(formatCurrency(paymentInitial))}</Button></form>}
                </div>
                <div className="rounded-md bg-muted p-4">
